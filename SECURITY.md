...
- **Telemetry.** On by default (opt-out) - the SDKs send a single anonymous
  install ping (no PII, no request content) to the ZNYX receiver, or to
  `ZNYX_TELEMETRY_URL` if you point it at a receiver you operate. Disable with
  `ZNYX_TELEMETRY=false`. Full details in [TELEMETRY.md](./TELEMETRY.md).
